...
{
	float x = pVec0->x / Vector2DLength(pVec0);
	float y = pVec0->y / Vector2DLength(pVec0);
	pResult->x = x;
	pResult->y = y;
}
...
{
	float x = (pVec0->x * pVec0->x);
	float y = (pVec0->y * pVec0->y);
	return  x + y;
}
...
{
	return Vector2DDistance(pVec0, pVec1)* Vector2DDistance(pVec0, pVec1);
}
